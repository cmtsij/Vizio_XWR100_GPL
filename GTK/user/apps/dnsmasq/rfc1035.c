/* dnsmasq is Copyright (c) 2000 Simon Kelley
   ...
   2008-5-30 Cybertan modify to support Router URL feature
*/
		   ...
		   char *mxtarget, unsigned int options, char *name)
{
	...
	time_t now = time(NULL);
	struct crec *crecp;
//add by michael to add router_url_address
//	char url[40];
//	char ipaddr[20];
//end by michael
  ...
  /* now process each question, answers go in RRs after the question */
	...
	for (q=0; q<qdcount; q++)
    {
      ...
      /* see if it's w.z.y.z.in-addr.arpa format */
		...
		ans = 1;
/*
//add by michael to add the router_url_address at 2008-3-5
///{
	FILE *fp;
        char tmp[60];
	char url_tmp[40];
        if(fp=fopen("/tmp/dns_url","r"))
        {
                fgets(tmp,sizeof(tmp),fp);
                strcpy(ipaddr,strtok(tmp,"#"));
                strcpy(url_tmp,strtok(NULL,"#")); 
                fclose(fp);
        }
 	if(!strstr(url_tmp,"://"))
		strcpy(url,url_tmp);
	else
		strcpy(url,url_tmp+7); //copy from behind the "http://"

///}
        if(!strncasecmp(name,url,strlen(url)))
        {		
                struct in_addr addr;
                unsigned long ttl = 0;
...
//willes add 2009/03/11 
		char *url;
		char *url_tmp = nvram_safe_get("url_address");
		char *ipaddr = nvram_safe_get("lan_ipaddr");
		
		if(!strstr(url_tmp,"://"))
			url = url_tmp;
		else
			url = url_tmp + 7; //copy from behind the "http://"
			
		if((qtype == T_A) && (!strncasecmp(name,url,strlen(url))))
        {		
            struct in_addr addr;
            unsigned long ttl = 0;
            inet_aton(ipaddr,&addr);
...
/*	if((qtype != T_AAAA) && (!strncasecmp(name, "VIZIO.HOME", 10) && strlen(name) == 10))
        {		
            struct in_addr addr;
            unsigned long ttl = 0;
            inet_aton(ipaddr,&addr);
