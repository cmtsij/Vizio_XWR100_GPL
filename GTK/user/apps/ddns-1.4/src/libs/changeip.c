...
#define DYNDNSHOST	"www.changeip.com"
#define DYNDNSHOST_IP	"213.186.33.34"
#define PORT		80

#define CACHEFILE	"/tmp/updatedd-changeip.ip"
#define MAXDATA 	512

#ifdef LONGOPT
#define USAGE	"\n                                Updatedd - \033[0;33;1mchangeip\033[0m\n\n"				\
...
int get_flags_changeip(struct arguments *args, int argc, char *argv[]);
int check_changeip(char *interface, struct ip_addresses *ip);
void update_changeip(int s, struct ip_addresses *ip, struct arguments *args);
void update_cache_changeip(struct ip_addresses *ip);
int get_local_cached_ip_changeip(struct ip_addresses *ip);
int check_server_msg_changeip(int s);

void
print_error_changeip(int priority, int instruction, int error_type, const char *message, ...) {

	va_list az;

	va_start(az, message);
	if(i_syslog_changeip == 1)
...
changeip(int argc, char *argv[]) {
	...
	while(1)
	{
		...
		if(get_flags_changeip(&args, argc, argv)) {
		...
		//if(check_changeip(args.interface, &ip)) {
			if((s = get_connection(DYNDNSHOST, PORT)) < 0)
				goto last;//print_error_changeip(LOG_ERR, EXIT, STANDARD, "get_connection() failed");

			update_changeip(s, &ip, &args);

			if(check_server_msg_changeip(s))
			{
				close(s);
				goto last;
				//print_error_changeip(LOG_ERR, EXIT, STANDARD, "unknown server message");
			} else {
				//update_cache_changeip(&ip);
				ip_record1 = ip_record2;
			}
			(void)close(s);
		//} else print_error_changeip(LOG_INFO, CONTINUE, STANDARD, "update is not necessary");
...
int
get_flags_changeip(struct arguments *args, int argc, char *argv[]) {
	...
	while(1) {
		...
		switch(c) {
			...
			case 'u':
				...
				print_error_changeip(LOG_ERR, EXIT, PERROR, "malloc() failed");
...
int
check_changeip(char *interface, struct ip_addresses *ip) {

	if((ip->real_ip.s_addr = get_ifaddr(interface)) == 0)
	    print_error_changeip(LOG_ERR, EXIT, STANDARD, "invalid interface: %s", interface);
	if(get_local_cached_ip_changeip(ip))
	    return 1;
...
int
get_local_cached_ip_changeip(struct ip_addresses *ip) {
	...
	if(!(fgets(ip_buffer, sizeof(ip_buffer) - 1, fp) &&
		...
		(void)fclose(fp);
		print_error_changeip(LOG_ERR, EXIT, STANDARD, "%s: invalid ip address", CACHEFILE);
	}
	(void)fclose(fp);
...
void
update_changeip(int s, struct ip_addresses *ip, struct arguments *args) {

	char *b64user;

	if(strlen(args->user) > 128)
	    print_error_changeip(LOG_ERR, EXIT, STANDARD, "username is too long");
	...
	(void)dprintf(s,	"GET https://changeip.com/nic/update?system=dyndns&hostname=%s HTTP/1.1\r\n"
				"Host: %s\r\n"
				"Authorization: basic %s\r\n"
				"User-Agent: %s %s - %s\r\n"
				"Connection: close\r\n"
				"Pragma: no-cache\r\n\n",
				args->hostname, DYNDNSHOST,
...
void
update_cache_changeip(struct ip_addresses *ip) {

	FILE *fp;

	(void)unlink(CACHEFILE);
	if((fp = fopen(CACHEFILE, "a")) == NULL)
	    print_error_changeip(LOG_ERR, EXIT, PERROR, "fopen() failed");
...
int
check_server_msg_changeip(int s) {
	...
	if(read(s, server_msg, sizeof(server_msg) - 1) < 0)
		return 1;
	    //print_error_changeip(LOG_ERR, EXIT, PERROR, "read() failed");
...
/*		
		for(n = 0; return_codes[n].code != NULL; n++) {
			if(strstr(server_msg, return_codes[n].code)) {
				...
				{
				    nvram_set("DDNSok", "0");
				    print_error_changeip(LOG_ERR, EXIT, STANDARD, return_codes[n].message);
				}
				else {
					nvram_set("DDNSok", "1");
					print_error_changeip(LOG_NOTICE, CONTINUE, STANDARD, return_codes[n].message);
					return 0;
				}
			}
		}
*/		
	    ...
	    print_error_changeip(LOG_ERR, EXIT, STANDARD, "Internal Server Error");
