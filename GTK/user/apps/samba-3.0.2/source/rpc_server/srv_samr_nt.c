...
NTSTATUS _samr_create_user(pipes_struct *p, SAMR_Q_CREATE_USER *q_u, SAMR_R_CREATE_USER *r_u)
{
	...
	if ( !pw ) {
		...
		if (*add_script) {
			...
			/* 
				Note: In our environment, we lose the '$' when we call out to another
				executable.  This apparently doesn't happen on most systems.  I
				could protect the '$' by prefixing it with a '\'.  Instead, I just put
				a protected '$' inside the script command line (see "add machine script"
				in smb.conf).
			GEW*/
...
NTSTATUS _samr_add_aliasmem(pipes_struct *p, SAMR_Q_ADD_ALIASMEM *q_u, SAMR_R_ADD_ALIASMEM *r_u)
{
	...
	if ((grp=sys_getgrgid(map.gid)) == NULL) {
		passwd_free(&pwd);
		return NT_STATUS_NO_SUCH_ALIAS;
	}
...
static int smb_delete_user(const char *unix_user)
{
	...
	/* 
		In our environment we lose the '$' when we call to outside executable.
		Apparently, this doesn't happen in most environments.  To solve the
		problem, we put a slash in front of the '$'.
	 GEW*/
	{
		size_t offset;
		
		/* Check for a trailing '$'.  The '$' is disappearing during the call. */
		offset = strlen( del_script ) - 1;
		if (del_script[ offset ] == '$')
		{
			del_script[ offset ] = '\\';
			del_script[ offset + 1 ] = '$';
			del_script[ offset + 2 ] = '\0';
		}
	}
...
NTSTATUS _samr_create_dom_group(pipes_struct *p, SAMR_Q_CREATE_DOM_GROUP *q_u, SAMR_R_CREATE_DOM_GROUP *r_u)
{
	...
	DEBUG(0,("create_dom_group: getgrgid enter\n"));
	if ((grp=sys_getgrgid(gid)) == NULL)
		return NT_STATUS_ACCESS_DENIED;
	DEBUG(0,("create_dom_group: getgrgid exit\n"));
	...
	DEBUG(0,("create_dom_group: add_init_entry enter\n"));
	if(!add_initial_entry(grp->gr_gid, sid_string, SID_NAME_DOM_GRP, name, NULL))
		return NT_STATUS_ACCESS_DENIED;
	DEBUG(0,("create_dom_group: add_init_entry exit\n"));
	...
	if ( is_user ) {
		...
		if ( pdb_enum_group_mapping(type, &mappings, &num_groups, False) && num_groups>0 ) {
			...
			for ( i=0; i<num_groups; i++ ) {
				...
				grp2 = sys_getgrgid(mappings[i].gid);
