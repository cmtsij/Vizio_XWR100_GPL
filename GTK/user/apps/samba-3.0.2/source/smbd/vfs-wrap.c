...
int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, int fd, SMB_OFF_T len)
{
	int result = -1;
	SMB_STRUCT_STAT st;
	SMB_STRUCT_STAT incoming_file;
	char c = 0;
	SMB_OFF_T currpos;
	unsigned long sam_st_size=0;
...
// Marked by Joseph 2009/05/18
// in order to prevent from allocating spaces before copying 
//sam modify +++ 2009.12.29 for fixed samba overwrite issue.
	/*
		Now, we limit overwritable size to 1GB.
	*/
	fstat(fd, &incoming_file);
	if(incoming_file.st_size <1024*1024*1024) //1GB
	{
		result = sys_ftruncate(fd, len);
		if (result == 0)
			goto done;
	}
//sam modify --- 2009.12.29
	...
	if (SMB_VFS_LSEEK(fsp, fd, len-1, SEEK_SET) != len -1)
		goto done;
	/* Don't create all file size, it will cause the timeout to client
			Gemtek Marmot Modified 20070709*/
	//if (SMB_VFS_WRITE(fsp, fd, &c, 1)!=1)
	if (SMB_VFS_WRITE(fsp, fd, &c, 0)!=0)
		goto done;
