...
#include "debug.h"
#include "pidfile.h"
#include "./gemtek_added_SRC/gemtek_jimmy_added_001.h"
...
static void perform_renew(void)
{
	...
	timeout = 0;
	_Gemtek_Jimmy_Added_001_H_Extra_SRC_001
}
...
Initial:	state = INIT_SELECTING;
	...
	for (;;) {
		...
		_Gemtek_Jimmy_Added_001_H_Extra_SRC_005
		if (tv.tv_sec > 0) {
			DEBUG(LOG_INFO, "Waiting on select...\n");
			// _Gemtek_Jimmy_Added_001_H_Extra_SRC_004
		...
		if (retval == 0) {
			...
			case REBINDING:
				if (*message == DHCPACK) {
					...
					_Gemtek_Jimmy_Added_001_H_Extra_SRC_002
					state = BOUND;
					change_mode(LISTEN_NONE);
