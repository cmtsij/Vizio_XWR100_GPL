...
void write_leases(void)
{
	...
	*buf_no = 0x00;
	...
	//if ( fp_no != NULL )
	//	fgets(buf_no,200,fp_no);
	
	for (i = 0; i < server_config.max_leases; i++) {
		if (leases[i].yiaddr != 0) {
			/* Marmot Add 20060802*/
			if (!(fp_no = fopen("/tmp/udhcpd.lease.no", "r"))) {
				LOG(LOG_ERR, "Unable to open /tmp/udhcpd.lease.no for reading");
				//return;
			}		
			Jimmy_Added_003_Extra_SRC_005
			/*Obsolete Jimmy_Added_003_Extra_SRC_006 -- Due to we use system uptime instead of real time*/
			...
			sprintf(lease_mac, "%02X:%02X:%02X:%02X:%02X:%02X", leases[i].chaddr[0] , leases[i].chaddr[1] , leases[i].chaddr[2] ,
	    leases[i].chaddr[3] , leases[i].chaddr[4] , leases[i].chaddr[5]); //Marmot Add 20060802
	    if(fp_no!=NULL)
			{		
				while(!feof(fp_no))
				{
					fgets(buf_no,20,fp_no);
					LOG(LOG_INFO, "lease_mac is %s, buf_no is %s", lease_mac, buf_no);
					if(!strstr(buf_no, lease_mac)) {
						LOG(LOG_INFO, "lease_mac is %s, buf_no is %s", lease_mac, buf_no);
						no_write = 0;
					}
					else {
	    ...
	    if(no_write == 0) {//Marmot Add 20060802
...
void read_leases(char *file)
{
	...
	while (i < server_config.max_leases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
		...
		//if (lease.yiaddr >= server_config.start && lease.yiaddr <= server_config.end) {
			...
			if (!server_config.remaining) lease.expires -= sys_time();//time(0);
