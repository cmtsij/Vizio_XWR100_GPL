...
{
		struct sysinfo current_sysinfo;
	
	  sysinfo(&current_sysinfo);
	  
	  return current_sysinfo.uptime;
...
{	
	...
	timeout_end = sys_time() + server_config.auto_time;//time(0) + server_config.auto_time;
	SomeExtraCodes_001
	while(1) { /* loop until universe collapses */
		...
		if (server_config.auto_time) {
			tv.tv_sec = timeout_end - sys_time();//timeout_end - time(0);
			tv.tv_usec = 0;
		}
		...
		case DHCPDECLINE:
			...
			if (lease) {
				memset(lease->chaddr, 0, 16);
				lease->expires = sys_time() + server_config.decline_time;//time(0) + server_config.decline_time;
			}			
			break;
		case DHCPRELEASE:
			DEBUG(LOG_INFO,"received RELEASE");
			if (lease) lease->expires = sys_time();//time(0);
			break;
		case DHCPINFORM:
