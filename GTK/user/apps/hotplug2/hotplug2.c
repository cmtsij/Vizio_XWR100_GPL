...
//	int unsupport_device, usb_unsupport_file_system;// for USB LED
//	char unsupport_device_buf[4];
...
end_rules:	
	...
	if ((key = ftok("/tmp/usb_storage_hotplug_queue", 'X')) == -1) {
        perror("ftok");
    }
	...
	msg_buf.mtype = 10;
	strcpy(msg_buf.mtext, "test_jopseph");
	...
	while (!terminate) {
		...
		recv_errno = errno;
		strcpy(buf, buffer);
//		fprintf(stderr, ">>> hotplug2 : %s (%i)\n", buf,__LINE__);
//oder marked +++
/*
		if(usb_interface = strstr(buf, "INTERFACE="))
		{
			//printf("buffer: %s\n", buf);
			//printf("%s\n", usb_interface);
			sscanf(usb_interface, "INTERFACE=%d/%d/%d", &usb_class, &usb_subclass, &usb_protocol);
			//printf("class=%d, subclass=%d, protocol=%d\n", usb_class, usb_subclass, usb_protocol);
			unsupport_device = atoi(nvram_safe_get("usb_unsupport_device"));
			//printf("usb_unsupport_device=%d\n", unsupport_device);
			usb_unsupport_file_system = atoi(nvram_safe_get("usb_unsupport_file_system"));
			//printf("usb_unsupport_file_system=%d\n", usb_unsupport_file_system);
			
			if( (usb_class != 8 && usb_class != 9 && usb_class != 3) && (usb_subclass != 6 && usb_subclass != 5))
			{
				if( !strncmp("add@", buf, 4) )
				{
					unsupport_device++;
					sprintf(unsupport_device_buf, "%d", unsupport_device);
					//printf("unsupport_device_count_add=%d\n", unsupport_device);
					nvram_set("usb_unsupport_device", unsupport_device_buf);
				}
				else
				if( !strncmp("remove@", buf, 7) )
				{
					unsupport_device--;
					sprintf(unsupport_device_buf, "%d", unsupport_device);
					//printf("unsupport_device_count_remove=%d\n", unsupport_device);
					nvram_set("usb_unsupport_device", unsupport_device_buf);
				}
			}
			...
			if( unsupport_device > 0 || usb_unsupport_file_system > 0) 
			{
				system("echo 1 > /proc/GPIO23;echo 1 > /proc/USB_BLINK;");//BELKIN_GPIO_USB_ERROR
				nvram_set("usb_error_led", "1");
			}
			else
			{
				int count= atoi(nvram_safe_get("device_count"));
				if( count > 0 )
					system("echo 0 > /proc/USB_BLINK;echo 0 > /proc/GPIO23;echo 1 > /proc/GPIO22;");//BELKIN_GPIO_USB_READY
				else
					system("echo 0 > /proc/USB_BLINK;echo 1 > /proc/GPIO23;echo 1 > /proc/GPIO22;");//BELKIN_GPIO_USB_DOWN
					
				nvram_set("usb_error_led", "0");
			}
		}
*/			
//oder marked ---
//oder added +++
//fprintf(stderr, "##########\n buf = %s \n ##########\n", buf);
		/* Add host number detect for Vendor_Product_Name */
		if(strncmp("add@/class/scsi_host/host", buf, 25) == 0 && buf[26] != '/') {
			char host_number[3]="";
			sprintf(host_number, "%c", buf[25]);
			nvram_set("USB_HOST_NUMBER", host_number);
		}

		if(strncmp("add@/block/sd", buf, 13) == 0 && buf[14] != '/') {
			nvram_set("USB_ACTION", "add");
			nvram_set("USB_ADD", strstr(buf, "sd"));
			if (msgsnd(msqid, (struct my_msgbuf *)&msg_buf, sizeof(msg_buf.mtext), 0) == -1) {
	            perror("msgsnd");
			}
			system("echo 0 > /proc/GPIO00");
		}
//oder added ---
		if(strncmp("remove@/block/sd", buf, 16) == 0 && buf[17] != '/') {
/*			char tmp[100];
			pid_t pid_1;
*/
			nvram_set("USB_REMOVE", strstr(buf, "sd"));
			nvram_set("USB_ACTION", "remove");
			/*strcpy(tmp, nvram_safe_get("USB_HOTPLUG_WRAPPER_PID"));
			sscanf(tmp, "%d", &pid_1);
			kill(pid_1, SIGUSR1); */
			if (msgsnd(msqid, (struct my_msgbuf *)&msg_buf, sizeof(msg_buf.mtext), 0) == -1) {
	            perror("msgsnd");
