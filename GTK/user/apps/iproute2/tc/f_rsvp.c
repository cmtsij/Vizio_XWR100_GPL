...
{
	my_printf("Usage: ... rsvp ipproto PROTOCOL session DST[/PORT | GPI ]\n");
	my_printf("                [ sender SRC[/PORT | GPI ]\n");
	my_printf("                [ classid CLASSID ] [ police POLICE_SPEC ]\n");
	my_printf("                [ tunnelid ID ] [ tunnel ID skip NUMBER ]\n");
	my_printf("Where: GPI := { flowlabel NUMBER | spi/ah SPI | spi/esp SPI |\n");
	my_printf("                u{8|16|32} NUMBER mask MASK at OFFSET}\n");
	my_printf("       POLICE_SPEC := ... look at TBF\n");
	my_printf("       FILTERID := X:Y\n");
}
