...
static int usage(void)
{
	my_printf("Usage: tc qdisc [ add | del | replace | change | get ] dev STRING\n");
	my_printf("       [ handle QHANDLE ] [ root | ingress | parent CLASSID ]\n");
	my_printf("       [ estimator INTERVAL TIME_CONSTANT ]\n");
	my_printf("       [ [ QDISC_KIND ] [ help | OPTIONS ] ]\n");
	my_printf("\n");
	my_printf("       tc qdisc show [ dev STRING ] [ingress]\n");
	my_printf("Where:\n");
	my_printf("QDISC_KIND := { [p|b]fifo | tbf | prio | cbq | red | etc. }\n");
	my_printf("OPTIONS := ... try tc qdisc add <desired QDISC_KIND> help\n");
	return -1;
}
...
int tc_qdisc_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "dev") == 0) {
			...
			if (req.t.tcm_parent) {
				my_printf("Error: \"ingress\" is a duplicate parent ID\n");
				return -1;
			}
	...
	if (q) {
		...
		if (argc) {
			...
			my_printf("Garbage instead of arguments \"%s ...\". Try \"tc qdisc help\".\n", *argv);
			return -1;
		}
	}
		       ...
		       void *arg)
{
	...
	if (tb[TCA_KIND] == NULL) {
		my_printf("print_qdisc: NULL kind\n");
		return -1;
	}
...
int tc_qdisc_list(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "dev") == 0) {
			...
			my_printf("What is \"%s\"? Try \"tc qdisc help\".\n", *argv);
			return -1;
		}
...
int do_qdisc(int argc, char **argv)
{
	...
	if (matches(*argv, "help") == 0)
		usage();
	my_printf("Command \"%s\" is unknown, try \"tc qdisc help\".\n", *argv);
	return -1;
}
