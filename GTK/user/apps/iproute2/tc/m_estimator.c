...
static void est_help(void)
{
	my_printf("Usage: ... estimator INTERVAL TIME-CONST\n");
	my_printf("  INTERVAL is interval between measurements\n");
	my_printf("  TIME-CONST is averaging time constant\n");
	my_printf("Example: ... est 1sec 8sec\n");
	return;
}
...
int parse_estimator(int *p_argc, char ***p_argv, struct tc_estimator *est)
{
	...
	if (tc_setup_estimator(A, time_const, est) < 0) {
		my_printf("Error: estimator parameters are out of range.\n");
		return -1;
	}
	if (show_raw)
		my_printf("[estimator i=%u e=%u]\n", est->interval, est->ewma_log);
	*p_argc = argc;
	*p_argv = argv;
	return 0;
}
