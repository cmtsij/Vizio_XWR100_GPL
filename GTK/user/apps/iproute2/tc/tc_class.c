...
static void usage(void)
{
	my_printf("Usage: tc class [ add | del | change | get ] dev STRING\n");
	my_printf("       [ classid CLASSID ] [ root | parent CLASSID ]\n");
	my_printf("       [ [ QDISC_KIND ] [ help | OPTIONS ] ]\n");
	my_printf("\n");
	my_printf("       tc class show [ dev STRING ] [ root | parent CLASSID ]\n");
	my_printf("Where:\n");
	my_printf("QDISC_KIND := { prio | cbq | etc. }\n");
	my_printf("OPTIONS := ... try tc class add <desired QDISC_KIND> help\n");
	return;
}
...
int tc_class_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	if (q) {
		if (q->parse_copt == NULL) {
			my_printf("Error: Qdisc \"%s\" is classless.\n", k);
			return 1;
		}
		...
		if (argc) {
			if (matches(*argv, "help") == 0)
				usage();
			my_printf("Garbage instead of arguments \"%s ...\". Try \"tc class help\".", *argv);
			return -1;
		}
	}
		       ...
		       struct nlmsghdr *n, void *arg)
{
	...
	if (tb[TCA_KIND] == NULL) {
		my_printf("print_class: NULL kind\n");
		return -1;
	}
...
int tc_class_list(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "dev") == 0) {
			...
			my_printf("What is \"%s\"? Try \"tc class help\".\n", *argv);
			return -1;
		}
...
int do_class(int argc, char **argv)
{
	...
	if (matches(*argv, "help") == 0)
		usage();
	my_printf("Command \"%s\" is unknown, try \"tc class help\".\n", *argv);
	return -1;
}
