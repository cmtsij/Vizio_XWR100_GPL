...
{
	my_printf("Usage: ... prio bands NUMBER priomap P1 P2...\n");
}
	...
	while (argc > 0) {
		if (strcmp(*argv, "bands") == 0) {
			...
			if (get_integer(&opt.bands, *argv, 10)) {
				my_printf("Illegal \"bands\"\n");
				return -1;
			}
			ok++;
		} else if (strcmp(*argv, "priomap") == 0) {
			if (pmap_mode) {
				my_printf("Error: duplicate priomap\n");
				return -1;
			}
			pmap_mode = 1;
			...
			if (get_unsigned(&band, *argv, 10)) {
				my_printf("Illegal \"priomap\" element\n");
				return -1;
			}
			if (band > opt.bands) {
				my_printf("\"priomap\" element is out of bands\n");
				return -1;
			}
			if (idx > TC_PRIO_MAX) {
				my_printf("\"priomap\" index > TC_PRIO_MAX=%u\n", TC_PRIO_MAX);
				return -1;
			}
