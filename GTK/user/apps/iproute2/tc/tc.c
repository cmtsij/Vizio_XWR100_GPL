...
{
	if (argc) {
		my_printf("Unknown qdisc \"%s\", hence option \"%s\" is unparsable\n", qu->id, *argv);
		return -1;
	}
	return 0;
}
...
static int parse_nofopt(struct filter_util *qu, char *fhandle, int argc, char **argv, struct nlmsghdr *n)
{
	...
	if (argc) {
		my_printf("Unknown filter \"%s\", hence option \"%s\" is unparsable\n", qu->id, *argv);
		return -1;
	}
	if (fhandle) {
		struct tcmsg *t = NLMSG_DATA(n);
		if (get_u32(&handle, fhandle, 16)) {
			my_printf("Unparsable filter ID \"%s\"\n", fhandle);
			return -1;
		}
		t->tcm_handle = handle;
	}
...
{
	my_printf("Usage: tc [ OPTIONS ] OBJECT { COMMAND | help }\n"
...
static int do_cmd(int argc, char **argv)
{
	...
	my_printf("Object \"%s\" is unknown, try \"tc help\".\n", 
		*argv);
	return -1;
}
...
static int makeargs(char *line, char *argv[], int maxargs)
{
	...
	for (cp = strtok(line, ws); cp; cp = strtok(NULL, ws)) {
		if (argc >= maxargs) {
			my_printf("Too many arguments to command\n");
			exit(1);
		}
		argv[argc++] = cp;
	}
...
static size_t getcmdline(char **linep, size_t *lenp, FILE *in)
{
	...
	while ((cp = strstr(*linep, "\\\n")) != NULL) {
		...
		if ((cc1 = getline(&line1, &len1, in)) < 0) {
			my_printf("Missing continuation line\n");
			return cc1;
		}
...
static int batch(const char *name)
{
	...
	if (strcmp(name, "-") != 0) {
		if (freopen(name, "r", stdin) == NULL) {
			printf("tc batch:Cannot open file \"%s\" for reading: %s=n",
				name, strerror(errno));
			return -1;
		}
	}
	...
	if (rtnl_open(&rth, 0) < 0) {
		printf("tc batch:Cannot open rtnetlink\n");
		return -1;
	}
	...
	while (getcmdline(&line, &len, stdin) != -1) {
		...
		if (do_cmd(largc-1, largv+1)) {//GTK -- Modify due to tc script included 'tc' 
			printf("tc batch:Command failed %s line:%d\n", name, lineno);
			ret = 1;
			if (!force)
				break;
		}
...
int main(int argc, char **argv)
{
	...
	while (argc > 1) {
		...
		if (matches(argv[1], "-stats") == 0 ||
			...
			if (argc < 3) {
				my_printf("Wrong number of arguments in batch mode\n");
				return -1;
			}
			batchfile = argv[2];
			argc--;	argv++;
		} else {
			my_printf("Option \"%s\" is unknown, try \"tc -help\".\n", argv[1]);
			return -1;
		}
		argc--;	argv++;
	}
	...
	tc_core_init();
	if (rtnl_open(&rth, 0) < 0) {
		my_printf("Cannot open rtnetlink\n");
		exit(1);
	}
