...
{
	my_printf("Usage: ... u32 [ match SELECTOR ... ] [ link HTID ] [ classid CLASSID ]\n");
	my_printf("               [ police POLICE_SPEC ] [ offset OFFSET_SPEC ]\n");
	my_printf("               [ ht HTID ] [ hashkey HASHKEY_SPEC ]\n");
	my_printf("               [ sample SAMPLE ]\n");
	my_printf("or         u32 divisor DIVISOR\n");
	my_printf("\n");
	my_printf("Where: SELECTOR := SAMPLE SAMPLE ...\n");
	my_printf("       SAMPLE := { ip | ip6 | udp | tcp | icmp | u{32|16|8} | mark } SAMPLE_ARGS\n");
	my_printf("       FILTERID := X:Y:Z\n");
}
...
static int parse_mark(int *argc_p, char ***argv_p, struct nlmsghdr *n)
{
	...
	if (get_u32(&mark.val, *argv, 0)) {
		my_printf("Illegal \"mark\" value\n");
		return -1;
	}
	NEXT_ARG();

	if (get_u32(&mark.mask, *argv, 0)) {
		my_printf("Illegal \"mark\" mask\n");
		return -1;
	}
	NEXT_ARG();

	if ((mark.val & mark.mask) != mark.val) {
		my_printf("Illegal \"mark\" (impossible combination)\n");
		return -1;
	}
...
static int parse_offset(int *argc_p, char ***argv_p, struct tc_u32_sel *sel)
{
	...
	while (argc > 0) {
		if (matches(*argv, "plus") == 0) {
			...
			sel->offoff = off;
			if (off%2) {
				my_printf("offset \"at\" must be even\n");
				return -1;
			}
...
static int u32_parse_opt(struct filter_util *qu, char *handle, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (matches(*argv, "match") == 0) {
			...
			if (get_u32_handle(&handle, *argv)) {
				my_printf("Illegal \"link\"\n");
				return -1;
			}
			if (handle && TC_U32_NODE(handle)) {
				my_printf("\"link\" must be a hash table.\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (get_u32_handle(&handle, *argv)) {
				my_printf("Illegal \"ht\"\n");
				return -1;
			}
			if (handle && TC_U32_NODE(handle)) {
				my_printf("\"ht\" must be a hash table.\n");
				return -1;
			}
			if (sample_ok)
			...
			if (parse_selector(&argc, &argv, &sel2.sel, n)) {
				my_printf("Illegal \"sample\"\n");
				return -1;
			}
			if (sel2.sel.nkeys != 1) {
				my_printf("\"sample\" must contain exactly ONE key.\n");
				return -1;
			}
	...
	if (order) {
		if (TC_U32_NODE(t->tcm_handle) && order != TC_U32_NODE(t->tcm_handle)) {
			my_printf("\"order\" contradicts \"handle\"\n");
			return -1;
		}
		t->tcm_handle |= order;
	}
