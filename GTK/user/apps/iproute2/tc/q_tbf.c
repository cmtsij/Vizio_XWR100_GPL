...
{
	my_printf("Usage: ... tbf limit BYTES burst BYTES[/BYTES] rate KBPS [ mtu BYTES[/BYTES] ]\n");
	my_printf("               [ peakrate KBPS ] [ latency TIME ]\n");
}
...
static int tbf_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (matches(*argv, "limit") == 0) {
			...
			if (opt.limit || latency) {
				my_printf("Double \"limit/latency\" spec\n");
				return -1;
			}
	...
	if (opt.rate.rate == 0 || !buffer) {
		my_printf("Both \"rate\" and \"burst\" are required.\n");
		return -1;
	}
	if (opt.peakrate.rate) {
		if (!mtu) {
	...
	if (opt.limit == 0 && latency == 0) {
		my_printf("Either \"limit\" or \"latency\" are required.\n");
		return -1;
	}
	...
	if (opt.peakrate.rate) {
		if ((Pcell_log = tc_calc_rtable(opt.peakrate.rate, ptab, Pcell_log, mtu, mpu)) < 0) {
			my_printf("TBF: failed to calculate peak rate table.\n");
			return -1;
		}
