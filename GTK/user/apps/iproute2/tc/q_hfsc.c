                     ...
                     struct nlmsghdr *n)
{
	...
	if (!(rsc_ok || fsc_ok || usc_ok)) {
		my_printf("HFSC: no parameters given\n");
		explain_class();
		return -1;
	}
	if (usc_ok && !fsc_ok) {
		my_printf("HFSC: Upper-limit Service Curve without "
...
hfsc_get_sc2(int *argcp, char ***argvp, struct tc_service_curve *sc)
{
	...
	if (umax != 0 && dmax == 0) {
		my_printf("HFSC: umax given but dmax is zero.\n");
		return -1;
	}
...
hfsc_get_sc(int *argcp, char ***argvp, struct tc_service_curve *sc)
{
	...
	if (sc->m1 == 0 && sc->m2 == 0) {
		my_printf("HFSC: Service Curve has two zero slopes\n");
		return -1;
	}
