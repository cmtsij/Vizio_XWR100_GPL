...
static void usage(void)
{
	my_printf("Usage: tc filter [ add | del | change | get ] dev STRING\n");
	my_printf("       [ pref PRIO ] [ protocol PROTO ]\n");
	my_printf("       [ estimator INTERVAL TIME_CONSTANT ]\n");
	my_printf("       [ root | classid CLASSID ] [ handle FILTERID ]\n");
	my_printf("       [ [ FILTER_TYPE ] [ help | OPTIONS ] ]\n");
	my_printf("\n");
	my_printf("       tc filter show [ dev STRING ] [ root | parent CLASSID ]\n");
	my_printf("Where:\n");
	my_printf("FILTER_TYPE := { rsvp | u32 | fw | route | etc. }\n");
	my_printf("FILTERID := ... format depends on classifier, see there\n");
	my_printf("OPTIONS := ... try tc filter add <desired FILTER_KIND> help\n");
	return;
}
...
int tc_filter_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "dev") == 0) {
			...
			if (req.t.tcm_parent) {
				my_printf("Error: \"root\" is duplicate parent ID\n");
				return -1;
			}
	...
	if (q) {
		...
		if (fhandle) {
			my_printf("Must specify filter type when using "
				"\"handle\"\n");
			return -1;
		}
		if (argc) {
			if (matches(*argv, "help") == 0)
				usage();
			my_printf("Garbage instead of arguments \"%s ...\". Try \"tc filter help\".\n", *argv);
			return -1;
		}
	}
	if (est.ewma_log)
	...
	if (d[0])  {
 	...
 	if (rtnl_talk(&rth, &req.n, 0, 0, NULL, NULL, NULL) < 0) {
		my_printf("We have an error talking to the kernel\n");
		return 2;
	}
			...
			void *arg)
{
	...
	if (n->nlmsg_type != RTM_NEWTFILTER && n->nlmsg_type != RTM_DELTFILTER) {
		my_printf("Not a filter\n");
		return 0;
	}
	len -= NLMSG_LENGTH(sizeof(*t));
	if (len < 0) {
		my_printf("Wrong len %d\n", len);
		return -1;
	}
	...
	if (tb[TCA_KIND] == NULL) {
		my_printf("print_filter: NULL kind\n");
		return -1;
	}
...
int tc_filter_list(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "dev") == 0) {
			...
			my_printf(" What is \"%s\"? Try \"tc filter help\"\n", *argv);
			return -1;
		}
...
int do_filter(int argc, char **argv)
{
	...
	if (matches(*argv, "help") == 0)
		usage();
	my_printf("Command \"%s\" is unknown, try \"tc filter help\".\n", *argv);
	return -1;
}
