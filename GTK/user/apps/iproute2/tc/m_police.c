...
{
	my_printf("Usage: ... police rate BPS burst BYTES[/BYTES] [ mtu BYTES[/BYTES] ]\n");
	my_printf("                [ peakrate BPS ] [ avrate BPS ]\n");
	my_printf("                [ ACTIONTERM ]\n");
	my_printf("Old Syntax ACTIONTERM := action <EXCEEDACT>[/NOTEXCEEDACT] \n"); 
	my_printf("New Syntax ACTIONTERM := conform-exceed <EXCEEDACT>[/NOTEXCEEDACT] \n"); 
	my_printf("Where: *EXCEEDACT := pipe | ok | reclassify | drop | continue \n");
	my_printf("Where:  pipe is only valid for new syntax \n");
}
...
int act_parse_police(struct action_util *a,int *argc_p, char ***argv_p, int tca_id, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		...
		if (matches(*argv, "index") == 0) {
			...
			NEXT_ARG();
			if (buffer) {
				my_printf("Double \"buffer/burst\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mtu) {
				my_printf("Double \"mtu/minburst\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (p.peakrate.rate) {
				my_printf("Double \"peakrate\" spec\n");
				return -1;
			}
	...
	if (p.rate.rate && !buffer) {
		my_printf("\"burst\" requires \"rate\".\n");
		return -1;
	}
	if (p.peakrate.rate) {
		if (!p.rate.rate) {
			my_printf("\"peakrate\" requires \"rate\".\n");
			return -1;
		}
		if (!mtu) {
			my_printf("\"mtu\" is required, if \"peakrate\" is requested.\n");
			return -1;
		}
	}
	...
	if (p.rate.rate) {
		if ((Rcell_log = tc_calc_rtable(p.rate.rate, rtab, Rcell_log, mtu, mpu)) < 0) {
			my_printf("TBF: failed to calculate rate table.\n");
			return -1;
		}
	...
	if (p.peakrate.rate) {
		if ((Pcell_log = tc_calc_rtable(p.peakrate.rate, ptab, Pcell_log, mtu, mpu)) < 0) {
			my_printf("POLICE: failed to calculate peak rate table.\n");
			return -1;
		}
