...
explain(void)
{
	my_printf("Usage: mirred <DIRECTION> <ACTION> [index INDEX] <dev DEVICENAME> \n");
	my_printf("where: \n");
	my_printf("DIRECTION := <ingress | egress>\n");
	my_printf("aCTION := <mirror | redirect>\n");
	my_printf("     : INDEX  is the specific policy instance id\n");
	my_printf("     : DEVICENAME is the devicename \n");
}
