...
static int parse_noaopt(struct action_util *au, int *argc_p, char ***argv_p, int code, struct nlmsghdr *n)
{
	...
	if (argc) {
		my_printf("Unknown action \"%s\", hence option \"%s\" is unparsable\n", au->id, *argv);
	} else {
		my_printf("Unknown action \"%s\"\n", au->id);
	}
	return -1;
}
...
bad_val:
	 ...
	 * cause enough pain */
	my_printf("parse_action: bad value (%d:%s)!\n",argc,*argv);
	return -1;
}
			   ...
			   void *arg)
{
	...
	if (NULL == tb[TCA_ACT_TAB]) {
		if (n->nlmsg_type != RTM_GETACTION)
			my_printf("do_print_action: NULL kind\n");
		return -1;
	}     
...
int tc_action_gd(int cmd, unsigned flags, int *argc_p, char ***argv_p)
{
	...
	while (argc > 0) {
		...
		a = get_action_kind(k);
		if (NULL == a) { 
			my_printf("Error: non existent action: %s\n",k);
			ret = -1;
			goto bad_val;
		}
		if (strcmp(a->id, k) != 0) {
		...
		argv +=1;
		if (argc <= 0) {
			my_printf("Error: no index specified action: %s\n",k);
			ret = -1;
			goto bad_val;
		}
...
int tc_act_list_or_flush(int argc, char **argv, int event)
{
	...
	if (event == RTM_DELACTION) { 
		...
		if (rtnl_talk(&rth, &req.n, 0, 0, NULL, NULL, NULL) < 0) {
			my_printf("We have an error flushing\n");
			return 1;
		}
...
int do_action(int argc, char **argv)
{
	...
	while (argc > 0) {
		...
		if (ret < 0) {
			my_printf("Command \"%s\" is unknown, try \"tc action help\".\n", *argv);
			return -1;
		}
	}
