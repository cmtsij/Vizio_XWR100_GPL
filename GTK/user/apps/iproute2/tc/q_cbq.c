...
{
	my_printf("Usage: ... cbq bandwidth BPS rate BPS maxburst PKTS [ avpkt BYTES ]\n");
	my_printf("               [ minburst PKTS ] [ bounded ] [ isolated ]\n");
	my_printf("               [ allot BYTES ] [ mpu BYTES ] [ weight RATE ]\n");
	my_printf("               [ prio NUMBER ] [ cell BYTES ] [ ewma LOG ]\n");
	my_printf("               [ estimator INTERVAL TIME_CONSTANT ]\n");
	my_printf("               [ split CLASSID ] [ defmap MASK/CHANGE ]\n");
}

static void explain(void)
{
	my_printf("Usage: ... cbq bandwidth BPS avpkt BYTES [ mpu BYTES ]\n");
	my_printf("               [ cell BYTES ] [ ewma LOG ]\n");
}
...
static int cbq_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	if (r.rate == 0) {
		my_printf("CBQ: bandwidth is required parameter.\n");
		return -1;
	}
	if (avpkt == 0) {
		my_printf("CBQ: \"avpkt\" is required.\n");
		return -1;
	}
	if (allot < (avpkt*3)/2)
		allot = (avpkt*3)/2;
	...
	if ((cell_log = tc_calc_rtable(r.rate, rtab, cell_log, allot, mpu)) < 0) {
		my_printf("CBQ: failed to calculate rate table.\n");
		return -1;
	}
	r.cell_log = cell_log;
	r.mpu = mpu;
...
static int cbq_parse_class_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "rate") == 0) {
			...
			if (prio > TC_CBQ_MAXPRIO) {
				my_printf("\"prio\" must be number in the range 1...%d\n", TC_CBQ_MAXPRIO);
				return -1;
			}
			wrr.priority = prio;
			wrr_ok++;
			...
			if (get_tc_classid(&fopt.split, *argv)) {
				my_printf("Invalid split node ID.\n");
				usage();
			}
			fopt_ok++;
			...
			if (err < 1) {
				my_printf("Invalid defmap, should be MASK32[/MASK]\n");
				return -1;
			}
			if (err == 1)
				fopt.defchange = ~0;
	...
	if (lss.change&(TCF_CBQ_LSS_OFFTIME|TCF_CBQ_LSS_MAXIDLE)) {
		if (lss.avpkt == 0) {
			my_printf("CBQ: avpkt is required for max/minburst.\n");
			return -1;
		}
		if (bndw==0 || r.rate == 0) {
			my_printf("CBQ: bandwidth&rate are required for max/minburst.\n");
			return -1;
		}
	}
	...
	if (wrr_ok) {
		...
		if (wrr.allot == 0) {
			my_printf("CBQ: \"allot\" is required to set WRR parameters.\n");
			return -1;
		}
	}
