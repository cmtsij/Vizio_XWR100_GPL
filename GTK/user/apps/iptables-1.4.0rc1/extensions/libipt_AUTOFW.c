...
{
	struct ipt_autofw_info *info = (struct ipt_autofw_info *)(*target)->data;

	switch (c) {
	case '1':
		if (!strcasecmp(optarg, "dnat"))
			info->type = IPT_AUTOFW_DNAT;
		else if (!strcasecmp(optarg, "in"))
			info->type = IPT_AUTOFW_IN;
		else if (!strcasecmp(optarg, "out"))
			info->type = IPT_AUTOFW_OUT;
		else
			exit_error(PARAMETER_PROBLEM,
      ...
      int numeric)
{
	...
	printf("AUTOFW ");
	if (info->type == IPT_AUTOFW_DNAT)
		printf("type:dnat ");
	else if (info->type == IPT_AUTOFW_IN)
		printf("type:in ");
	else if (info->type == IPT_AUTOFW_OUT)
		printf("type:out ");
...
static struct iptables_target trigger = {
	...
	.size		= IPT_ALIGN(sizeof(struct ipt_autofw_info)),
	.userspacesize	= IPT_ALIGN(sizeof(struct ipt_autofw_info)),
	.help		= help,
	.parse		= parse,
