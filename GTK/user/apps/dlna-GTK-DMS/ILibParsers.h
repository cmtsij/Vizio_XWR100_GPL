...
#define sem_post(x) ILibSymbian_SignalSemaphore(x)
	...
	#endif
#elif defined(_POSIX)
	// Gemtek +++ 20090324 --- we cannot rely on original timestamp source as we have NTP concern (WAN up)
	#define gtk_gettimeofday(tp,tzp) ILibGtkGetTimeOfDay(tp)	
#endif
...
void ILibGtkGetTimeOfDay( struct timeval *tv );
/* \} */
