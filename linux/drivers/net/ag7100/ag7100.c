...
/* +++ Gemtek Added */
extern int ag7100_proc_fs_init(void);
extern int ag7100_qos_proc_fs_init(void);
/* --- Gemtek Added */
...
//sam add +++ 2008.09.19 for fixed device can not send packet and the size is 516
//origin        if (*len < (tx_len_per_ds + 4))
...
/* +++ Gemtek Added for LAN MAC plus 20081118 +++ */
void MACplusplus(unsigned char* MAC_ori)
{
	unsigned char temp_MAC[6] = {0};
	unsigned long ori_mac = 0, new_mac = 0;
	
  printk("MAC PLUS %d\n",__LINE__);
	temp_MAC[0] = MAC_ori[0];
	temp_MAC[1] = MAC_ori[1];
	temp_MAC[2] = MAC_ori[2];
	temp_MAC[3] = MAC_ori[3];
	temp_MAC[4] = MAC_ori[4];
	temp_MAC[5] = MAC_ori[5];
  printk("MAC PLUS %d\n",__LINE__);
						
	ori_mac = temp_MAC[2] * 256 * 256 * 256 + temp_MAC[3] * 256 * 256 + temp_MAC[4] * 256 + temp_MAC[5];
	
  printk("MAC PLUS %d\n",__LINE__);
	if(ori_mac != 0xFFFFFFFF)
	{
  printk("MAC PLUS %d\n",__LINE__);
		new_mac = ori_mac + 1;
		
		MAC_ori[0] = temp_MAC[0];
		MAC_ori[1] = temp_MAC[1];
		MAC_ori[2] = (new_mac >> 24) & (0xFF);
		MAC_ori[3] = (new_mac >> 16) & (0xFF);
		MAC_ori[4] = (new_mac >> 8) & (0xFF);
		MAC_ori[5] = new_mac & (0xFF);
	}
	else
	{
  printk("MAC PLUS %d\n",__LINE__);
		long mac_head = 0;
		mac_head = temp_MAC[0] * 256 + temp_MAC[1];
		mac_head++;
		
		MAC_ori[0] = (mac_head >> 8) & 0xFF;
		MAC_ori[1] = mac_head & 0xFF;
		MAC_ori[2] = 0x00;
		MAC_ori[3] = 0x00;
		MAC_ori[4] = 0x00;
		MAC_ori[5] = 0x00;
  ...
  printk("MAC PLUS %d\n",__LINE__);
  printk("MAC PLUS  %02x:%02x:%02x:%02x:%02x:%02x \n",
...
ag7100_get_default_macaddr(ag7100_mac_t *mac, u8 *mac_addr)
{
    ...
    */
    ...
    else
    {
        ...
        /* Use the EEPROM MAC Address as the Default Address Gemtek Added 20081209 */
        memcpy(mac_addr, eep_mac_addr, 6);
    }

/* +++ Gemtek Added for LAN MAC plus 20081118 +++ */
    MACplusplus(mac_addr);
/* --- Gemtek Added for LAN MAC plus 20081118 --- */
}
...
ag7100_init(void)
{
    ...
    */
    /* +++ Gemtek Added +++ */
    printk(MODULE_NAME ": create proc file\n");
		ag7100_proc_fs_init();
		ag7100_qos_proc_fs_init();
    /* --- Gemtek Added --- */
    ...
    */
    ...
    for(i = 0; i < AG7100_NMACS; i++)
    {
        ...
        */
        /* +++ Gemtek Modify : Don't reset the PHY 20090120 +++ */
        //ar7100_reg_rmw_set(AR7100_RESET, mask);
        //mdelay(100);
        //ar7100_reg_rmw_clear(AR7100_RESET, mask);
        //mdelay(100);
