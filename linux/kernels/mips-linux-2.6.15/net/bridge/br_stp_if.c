/*	Feature description :
	If the specific i/f does exist, the br0 always uses the MAC address of that specific i/f as its own.
	added by GTK */
#define	_Br0UseTheFixedMacAddress_
#ifdef		_Br0UseTheFixedMacAddress_
#define	_TheTargetInterfaceName_				"eth0"
#endif		/* end of _Br0UseTheFixedMacAddress_ */
...
void br_stp_recalculate_bridge_id(struct net_bridge *br)
{
	...
	list_for_each_entry(p, &br->port_list, list) {
		...
		if ( NULL != p->dev ) {
			if ( NULL != p->dev->name ) {
				if ( !memcmp ( p->dev->name , _TheTargetInterfaceName_ , strlen ( _TheTargetInterfaceName_ ) ) ) {
					addr = p->dev->dev_addr;
					break;
				}	/* memcmp */
			}	/* ( NULL != p->dev->name ) */
