...
#define DUMP_TUPLE_GRE(x) printk("%s->%s(%d),%u.%u.%u.%u:0x%x -> %u.%u.%u.%u:0x%x\n", __FILE__, __FUNCTION__, __LINE__, \
		  ...
		  unsigned int hooknum)
{
	...
	DEBUGP("%s->%s(%d),======================================================\n", __FILE__, __FUNCTION__, __LINE__ );
	DUMP_TUPLE_GRE(conntrack->tuplehash[0].tuple);
	DUMP_TUPLE_GRE(conntrack->tuplehash[IP_CT_DIR_REPLY].tuple);
	DUMP_TUPLE_GRE(conntrack->tuplehash[2].tuple);
	DEBUGP("%s->%s(%d),======================================================\n", __FILE__, __FUNCTION__, __LINE__ );
	  ...
	  enum ip_nat_manip_type maniptype)
{
	...
	DEBUGP("%s->%s(%d),NAT proto name=%s,NAT proto protonum=%d\n", __FILE__, __FUNCTION__, __LINE__, p->name, p->protonum );
...
#ifdef CONFIG_IP_NF_NAT_PROTO_GRE
	ip_nat_protos[IPPROTO_GRE] = &gre;
#endif
//sam add ---
	write_unlock_bh(&ip_nat_lock);
