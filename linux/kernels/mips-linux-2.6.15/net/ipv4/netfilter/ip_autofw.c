		 ...
		 struct ip_nat_info *info)
{
	...
	u_int16_t *dst_u_all_ptr16 = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all;
	...
	if (HOOK2MANIP(hooknum) == IP_NAT_MANIP_DST) {
		...
		memcpy(&mr.range[0].min, &port, sizeof(port));
		memcpy(&mr.range[0].max, &port, sizeof(port));
		/*
...
autofw_expect(struct ip_conntrack *ct)
{
	...
	//port = ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all);
	port = ntohs(*dst_u_all_ptr16);
	DEBUGP("autofw_expect: port is %d\n", port);
	DEBUGP("autofw_expect: port[0] is %d\n", ntohs(ct->master->help.exp_autofw_info.dport[0]));
	DEBUGP("autofw_expect: port[1] is %d\n", ntohs(ct->master->help.exp_autofw_info.dport[1]));
	if (port >= ntohs(ct->master->help.exp_autofw_info.dport[0]) &&
	    port <= ntohs(ct->master->help.exp_autofw_info.dport[1])) {
