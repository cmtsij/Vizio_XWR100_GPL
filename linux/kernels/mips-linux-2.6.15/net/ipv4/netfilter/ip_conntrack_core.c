...
#ifdef CONFIG_IP_NF_PRIVILEGE_CONNTRACK
#define CONNTRACK_PORT_ARRAY_SIZE	12
int general_traffic_conntrack_max = 200;
static __u16 privilege_conntrack_port[CONNTRACK_PORT_ARRAY_SIZE]={80,23,3128,1863,21,25,110,443,53,67,68,69};
#endif
	       ...
	       struct sk_buff *skb)
{
	...
	if(general_traffic_conntrack_max &&
		atomic_read(&ip_conntrack_count) > general_traffic_conntrack_max)
	{
		/* check ports */
		int i, passable = 0;

		if(protocol->proto == IPPROTO_TCP ||protocol->proto == IPPROTO_UDP )
		{
			//printk(KERN_WARNING"conn %d exceeds limit %d",
			//	ip_conntrack_count, general_traffic_conntrack_max);
			
			struct tcphdr *tcph = skb->h.th;
			/* I dont know why the skb tcp header points to wrong place */
			unsigned short port = *((unsigned short *)(skb->nh.iph)+11);
			...
			//printk(KERN_WARNING"conn %d exceeds limit %d port is %d passable is %d\n",
			//	ip_conntrack_count, general_traffic_conntrack_max, port, passable);
		}else
			passable = 1;
...
#endif /* CONFIG_IP_NF_PRIVILEGE_CONNTRACK */
