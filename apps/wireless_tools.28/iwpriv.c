		...
		int		priv_num)	/* Number of descriptions */
{
  ...
  if((priv[k].get_args & IW_PRIV_TYPE_MASK) &&
	...
	case IW_PRIV_TYPE_CHAR:
	  ...
	  buffer[n] = '\0';
	  printf("%s\n", buffer);
/* +++ Gemtek Marmot Add to set the country code to nvram value 20081103 +++ */
			char buf[36];
		  if(!strncmp(cmdname, "getCountry", strlen("getCountry")))
		  {
		  	sprintf(buf, "/sbin/nvram set %s_countrycode=%s", ifname, buffer);
		  	system(buf);
		  }
/* --- Gemtek Marmot Add to set the country code to nvram value 20081103 --- */
	  break;
